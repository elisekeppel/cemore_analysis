...
sp = "humpback whale"
sf <- all_ap_sf %>% filter(Species == sp)# %>% filter(iteration<=19)

# sf <- sf %>% filter(!is.na(Reticle))
df <- sf %>% data.frame()# %>% filter(iteration<=19)
...
# par(mfrow=c(1,2))
# terra::hist(sf2$distance, xlab="Distance (km)", breaks=seq(0,max(sf$distance),len=30),
#      main=paste0(sp, " to ", survey_title2, "-30bins"))#, xlim = range(c(0,4)))  
# terra::hist(sf2$distance, xlab="Distance (km)",breaks=seq(0,max(sf$distance),len=60),
#      main=paste0(sp, " to ", survey_title2, "-60bins"))#, xlim = range(c(0,4)))
```
...
# ret2 <- sf2 %>% filter(!is.na(Reticle))

par(mfrow=c(1,2))
hist(ret$distance, xlab="Distance (km)",breaks=seq(0,max(ret$distance),len=30),
     main=paste0("To ", survey_title, " - 30bins - ret only"))
# hist(ret2$distance, xlab="Distance (km)",breaks=seq(0,max(ret$distance),len=30),
#      main=paste0("To ", survey_title2, " - 30bins - ret only"))
...
cat(paste("Half normal to ", survey_title, " at 3, 2 and 1.5 km truncation"))
...
# hn.3x <-ds(df2, key = "hn", truncation = 3)
# hn.2x <-ds(df2, key = "hn", truncation = 2)
# hn.1.5x <-ds(df2, key = "hn", truncation = 1.5)
save(hn.3,
hn.2,
hn.1.5,
# hn.3x,
...
# cat(paste("Half normal to ", survey_title2, " at 3, 2 and 1.5 km truncation"))
# 
# # plot_df(sp, hn.6x)
# # plot_df(sp, hn.5x)
# # plot_df(sp, hn.4x)
...
# plot_df(sp, hn.1.5x)
# 
# gof_ds(hn.3x, main="hn.3x")
# gof_ds(hn.2x, main="hn.2x") 
# gof_ds(hn.1.5x, main="hn.1.5x")
# 
...
cat(paste("Hazard rate to ", survey_title, " at 3, 2 and 1.5 km truncation"))
...
# hr.3x <-ds(df2, key = "hr", truncation = 3)
# hr.2x <-ds(df2, key = "hr", truncation = 2)
# hr.1.5x <-ds(df2, key = "hr", truncation = 1.5)
...
#######
# cat(paste("Hazard rate to ", survey_title2, " at 3, 2 and 1.5 km truncation"))
# 
# par(mfrow=c(1,3))
# plot_df(sp, hr.3x)
# plot_df(sp, hr.2x) # TO DO try adding showpoints=FALSE into plot() in plot_df(), particularly for models wo covariates
# plot_df(sp, hr.1.5x)
# 
# par(mfrow=c(1,3))
# gof_ds(hr.3x, main="hr.3x")
# gof_ds(hr.2x, main="hr.2x") 
# gof_ds(hr.1.5x, main="hr.1.5x")
```
...
##################
# cat(paste("Uniform to ", survey_title2, " at 3, 2 and 1.5 km truncation"))
# 
# un.3x <-ds(df2, key = "unif", truncation = 3)
# un.2x <-ds(df2, key = "unif", truncation = 2)
# un.1.5x <- ds(df2, key ="unif", truncation = 1.5)
# 
# par(mfrow=c(1,3))
# plot_df(sp, un.3x  ) 
# plot_df(sp, un.2x  )
...
# 
# par(mfrow=c(1,3))
# gof_ds(un.3x  , main="un.3x") 
# gof_ds(un.2x  , main="un.2x") 
# gof_ds(un.1.5x, main="un.1.5x")
```
...
```{r beau}
survey_title
df2k <- df %>% filter(distance<=2)
table(df2k$Beaufort)

# survey_title2
# df2k_old <- df2 %>% filter(distance <=2)
# table(df2k_old$Beaufort)
...
# all_effort_lines_sum2 <- all_effort_lines2 %>% mutate(length_km = as.numeric(st_length(geometry))/1000) %>% as.data.frame() %>% mutate(Beaufort = factor(Beaufort, levels = seq(0,max(Beaufort))))
...
#      xlab="Beaufort")
# survey_title2
# ggplot(all_effort_lines_sum2, aes(x=Beaufort, y=length_km)) + 
#   geom_bar(stat = "identity") +
#   ggtitle("Distance surveyed per Beaufort level")
# ggplot(df_sum2, aes(x=Beaufort, y=Observations)) + 
#   geom_bar(stat = "identity") +
  # ggtitle("Observations per Beaufort level")

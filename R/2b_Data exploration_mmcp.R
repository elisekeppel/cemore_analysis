...
surveyid = paste0("mmcp_", year, tolower(month_abb), "_", vessel)
...
# ----------------------------------------------------------------------
# year month_abb   iteration         SurveyID            objectives
# 1 2022       Jul          B5  mmcp_2022jul_CC Line-transect surveys
# 2 2022       Jul         A5a  mmcp_2022jul_GN Line-transect surveys
# 3 2022       Aug AB_a3_AB_b2  mmcp_2022aug_GN Line-transect surveys
# 4 2022       Aug          B6 mmcp_2022aug_GN2 Line-transect surveys
# 5 2022       Aug         A5a  mmcp_2022aug_CC Line-transect surveys
# 6 2022       Sep         A7a  mmcp_2022sep_GN Line-transect surveys
# 7 2022       Sep          B7  mmcp_2022sep_TJ Line-transect surveys
...
dir <- file.path("C:/Users/KeppelE/Documents/CeMoRe/Analysis/cemore_analysis",paste0("OUTPUT FILES ",data.source),"dataEffort table")
# effort <- readRDS(file.path("output_mmcp", "effort", "effort_2022_09.rds"))
#-------------------------------------------------------------------
# ----------------------- SAVE INDIV SURVEYS --------------------
#-------------------------------------------------------------------
...
{folder <- "output_mmcp"
saveRDS(effort, paste0(folder, "/effort/effort_", year, "_",month,vessel,".rds"))
saveRDS(effort_lines, paste0(folder, "/effort_lines/effort_lines_", year, "_",month,vessel, ".rds"))
saveRDS(ap_sf, paste0(folder, "/effort_sgt/effort_sgt_", year, "_",month,vessel, ".rds"))}

#-------------------------------------------------------------------
# ----------------------- SAVE ALL SURVEYS --------------------
#-------------------------------------------------------------------
folder <- "output_mmcp"
all_effort <- load_effort(year, month, single=F, data.source = "mmcp")
all_effort_lines <- get_effort_lines(all_effort)
all_ap_sf <- load_sightings(year, month, single=F, data.source = "mmcp")

saveRDS(all_effort, paste0(folder, "/all_effort/all_effort_to_", year, "_",month, ".rds"))
saveRDS(all_effort_lines, paste0(folder, "/all_lines/all_effort_lines_to_", year, "_",month, ".rds"))
saveRDS(all_ap_sf, paste0(folder, "/all_sgt//all_effort_sgt_to_", year, "_",month, ".rds"))
...
{xmin=min(effort$Longitude)-0.5
xmax=max(effort$Longitude)+0.5
ymin= min(effort$Latitude)-0.5
ymax= max(effort$Latitude)+0.5
coords <- ggplot2::coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax), crs = sf::st_crs(4326)) }# 
...
#         Plot survey
#--------------------------------------
# bathy <- getNOAA.bathy(-132.5, -127.5,55.5, 51.5,res=1, keep=TRUE) %>% # North coast Franklin
#   fortify(b)
...
plot_survey(Save = T,
            file_name = paste0("output_maps_mmcp/",year,"-",month,"-",vessel,".png"),
            border = F,
...
effort <- all_effort
ap_sf <- all_ap_sf
plot_survey(single_survey=F, plot_effort = T, species = "humpback whale", monthly=T)
#-------------------------------------------
